...
enum BP_TYPE {
    BP_FNBT, BP_BIMODAL, BP_SAG, BP_GAG, BP_GSHARE,
    BP_HYBRID_SAG_GAG, BP_HYBRID_MAJORITY, BP_HYBRID_TOURNAMENT, BP_COUNT
};
...
VOID InitPredictors() {
    ...
    for (int i = 0; i < BIMODAL_SIZE; i++) bimodal_pht[i] = 2;
...
INT32 FindBTBEntry(BTB_ENTRY btb[BTB_WAYS], ADDRINT pc) {
    for (INT32 i = 0; i < BTB_WAYS; i++) {
        if (btb[i].valid && btb[i].tag == pc) return i;
    }
    return -1;
}
...
VOID UpdateLRU(BTB_ENTRY btb[BTB_WAYS], INT32 used_way) {
    for (INT32 i = 0; i < BTB_WAYS; i++) {
        if (i != used_way) btb[i].lru++;
    }
    if (used_way != -1) btb[used_way].lru = 0;
}
...
VOID AnalyzeBranch(ADDRINT pc, BOOL taken, BOOL isForward) {
    for (INT32 i = 0; i < BP_COUNT; i++) {
        ...
        switch (i) {
            case BP_FNBT: prediction = !isForward; break; // Forward not-taken, backward taken
            case BP_BIMODAL: prediction = PredictBimodal(pc); break;
            case BP_SAG: prediction = PredictSAg(pc); break;
            case BP_GAG: prediction = PredictGAg(); break;
            case BP_GSHARE: prediction = PredictGshare(pc); break;
            case BP_HYBRID_SAG_GAG: prediction = PredictHybridSAgGAg(pc); break;
            case BP_HYBRID_MAJORITY: prediction = PredictHybridMajority(pc); break;
            case BP_HYBRID_TOURNAMENT: prediction = PredictHybridTournament(pc); break;
        }
        if (prediction != taken) {
            bp_stats[i].mispredictions++;
            if (isForward) bp_stats[i].forward_misp++;
            else bp_stats[i].backward_misp++;
        }
        bp_stats[i].total++;
        if (isForward) bp_stats[i].forward++;
        else bp_stats[i].backward++;
    }
